...
parsing the whole document. BisonDB implements its own codec (the encoder, decoder, and the
...
`std::map<std::string, Value>` would be wrong; internally it is a vector of key/value pairs
