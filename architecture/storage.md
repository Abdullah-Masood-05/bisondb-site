...
Updates do not modify bytes in place — ever. An update appends a new PUT for the same
...
record**. A torn tail is defined as not-yet-written data, not corruption:
...
The key observation: the torn record was never acknowledged to the client (responses are
...
- **Update-in-place (heap files)** needs free-space management, record relocation when
  documents grow, and write-ahead logging to make any of it crash-safe — a WAL *plus* the
  files it protects. The append-only log **is** its own WAL.
...
- The trade: reads need an index lookup plus a seek (no clustering), deleted space
