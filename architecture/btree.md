...
on-disk B+Tree, without `std::map` standing in for the hard part, and without using a storage library. It maps
...
Queries like `{pop: {$gte: 40000}}` are *range* queries, which a hash index cannot answer
...
Both node kinds share the same layout: a 12-byte header, a growing slot array, and cells
...
order Null < Numbers < String < ObjectId < Bool < DateTime) followed by a payload:
...
Keys cap at **512 encoded bytes**. Longer values (as well as NaN, documents, and arrays) simply
are not indexed. Those documents are still found by full scans, and index builds count them
as skipped.

### The double sign-flip trick

IEEE 754 doubles sort like their bit patterns for positive numbers, but negative
numbers have the sign bit set (sorting them above positives unsigned) and grow
*downward* in magnitude as their bit patterns grow. Two transformations address this:

- sign bit **clear** (positive): set the sign bit → moves positives above all negatives
- sign bit **set** (negative): flip **all 64 bits**, which reverses the negatives' order and
...
(−0.0 is normalized to +0.0 first so the two zeros collide to match numeric equality.)
Stored big-endian, `memcmp` sorts doubles numerically. The string escaping ensures `"a" < "a\0" < "aa"` remains correct under
...
into the leaf. When a page cannot fit the new cell even after compacting its holes, it
splits:
...
Leaf splits *copy* the separator up (the key stays in the leaf because range scans need every key
at leaf level); internal splits *move* their median up. A root split allocates a new root
with one separator, which is the only way this tree gets taller; the tree grows perfectly
balanced, by construction.

## Deletion: deliberately lazy

Textbook B-trees rebalance on underflow: they borrow from a sibling, merge, and potentially
...
fully-empty pages through the free list. This leaves the worst case of a long-lived,
