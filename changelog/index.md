...
|---|---|---|
| BisonDB engine | **v1.2.0** (TLS and authentication) | v2 |
| Prairie (GUI) | **v1.2.1** (UI/UX and motion) | v2 |
...
- **Tooling:** `bisonc tls gen-cert --out-dir <dir>` writes `cert.pem` + a `0600` `key.pem`, which is
...
- **Wire protocol $\rightarrow$ v2.** New `authenticate`, `authenticateToken`, `logout`, `createUser`,
  ...
  reports a `security` block. This is breaking for v1 clients (which never authenticate); they
...
The Compass-style desktop GUI (Tauri + React). Tracks the engine's wire protocol, which is now on
**v2** (auth and TLS).
...
and motion, focusing on restraint over effects, using the same dark amber-on-neutral theme.

- A design-token layer (surfaces, one accent, radii, elevation, type scale) on the **Inter**
  typeface; cohesive buttons, inputs, cards, modals, and toasts.
- A small **motion** system (framer-motion): modal and toast transitions, route cross-fades, a
...
- **TLS** with the server's verification modes (system trust, CA or self-signed file,
  SHA-256 pin, or insecure), a workspace lock indicator, and local databases that run the
  bundled sidecar over a pinned self-signed cert (encrypted and verified, no login).
...
- **Admin Users panel** (create, reset password, or drop with roles) and a **role-aware UI**
  (read-only users have write controls disabled).
- Recent connections remember username and TLS prefs, never secrets.
...
local database no longer pops a visible `bisond.exe` console window on Windows, as the sidecar
